...
class AgentBundleBuildTests(unittest.TestCase):
    def test_reproducible_archives(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with zipfile.ZipFile(output / "tailor-cv-agent-skill-v1.0.0-beta.2.zip") as archive:
                self.assertIn("tailor-cv/SKILL.md", archive.namelist())
            with zipfile.ZipFile(output / "tailoring-ats-cvs-claude-plugin-v1.0.0-beta.2.zip") as archive:
