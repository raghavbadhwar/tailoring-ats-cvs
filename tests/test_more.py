...
from ats_agent.evidence import EvidenceItem, EvidenceLedger, EvidenceSource, SourceFragment, build_evidence_ledger, detect_ownership, ownership_rank
...
class MoreTests(unittest.TestCase):
    def test_evidence_model_rejects_duplicates_and_source_mismatch(self):
        item = EvidenceItem(
            id="E1", candidate_id="a", text="Built Python workflows", source="resume",
            source_file="resume.txt", source_span="line 1", line_number=1,
            paragraph_index=None, part="text", ownership="direct",
        )
    ...
    def test_source_fragments_preserve_paragraph_anchor(self):
        source = EvidenceSource(
            source="resume", source_file="resume.docx", text="Built Python workflows",
            fragments=(SourceFragment("word/document.xml", 4, None, "Built Python workflows"),),
        )
    ...
    def test_requirement_extraction_more_gate_types(self):
        requirements = extract_requirements(
            "A minimum CGPA of 8.0 is required. This is an on-site role with 25% travel. "
            "React, SQL, Docker, and stakeholder management are preferred."
        )
        kinds = {r["kind"] for r in requirements}
        self.assertTrue({"minimum_grade", "work_mode", "travel", "skill"}.issubset(kinds))
        terms = {t for r in requirements for t in r["normalized_terms"]}
        self.assertTrue({"react", "sql", "docker", "stakeholder management"}.issubset(terms))

    def test_map_marks_unknown_skill_unsupported(self):
        ledger = build_evidence_ledger("a", [EvidenceSource("resume", "r.txt", "- Built Excel analysis workflows.")])
        requirements = extract_requirements("Kubernetes is required.")
        # Kubernetes is not in taxonomy, so no false requirement should be invented.
        self.assertFalse(any("kubernetes" in r.get("normalized_terms", []) for r in requirements))
    ...
    def test_validation_rejects_unsupported_employer_and_duplicate_ids(self):
        ledger = build_evidence_ledger("a", [EvidenceSource("resume", "r.txt", "- Contributed to Python workflows with 42 tests.")])
        item = ledger.items[0]
        change = {
            "id": "C1", "operation": "replace_span", "supported": True,
            "evidence_ids": [item.id], "expected_text": item.text,
            "replacement_text": "Contributed to Python workflows for OpenAI with 42 tests.",
        }
        with self.assertRaisesRegex(ValueError, "organization"):
            validate_change(change, ledger)
        safe = {**change, "replacement_text": "Supported Python workflows with 42 tests."}
    ...
    def test_text_insert_and_rebuild_docx(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            patch_document(source, output, [{
                "id": "C1", "operation": "insert_after", "expected_text": "PROJECTS",
                "replacement_text": "Built Python workflow automation with 42 tests.",
                "anchor": {"heading": "PROJECTS", "line_number": 3},
            }], mode="rebuild")
            loaded = load(output)
    ...
    def test_html_rtf_and_unsupported_ingestion(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            html_file.write_text("<h1>SUMMARY</h1><p>Built Python workflows with 42 tests.</p>", encoding="utf-8")
            self.assertIn("Built Python", load(html_file)["text"])
            rtf = root / "a.rtf"
            rtf.write_text(r"{\rtf1 SUMMARY\par Built Python workflows with 42 tests.}", encoding="utf-8")
    ...
    def test_report_is_explainable_not_universal_score(self):
        ledger = build_evidence_ledger("a", [EvidenceSource("resume", "r.txt", "- Built Python workflows with 42 tests.")])
        report = career_report("SUMMARY\nAI product candidate\n- Built Python workflows with 42 tests.", "Python is required.", ledger=ledger)
    ...
    def test_reporting_handles_empty_sections(self):
        proposal = {"candidate_id": "a", "status": "draft", "source": "resume.txt", "requirements": [], "requirement_evidence": [], "hard_gates": [], "changes": [], "evidence_ledger": []}
        self.assertIn("No deterministic hard gate", proposal_markdown(proposal))
    ...
    def test_apply_rejects_no_approval_and_invalid_mode(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            manifest.write_text(json.dumps({"proposal": str(proposal_path)}), encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "no changes"):
                apply_manifest(manifest)
            change = next(c for c in proposal["changes"] if c.get("supported"))
            manifest.write_text(json.dumps({"proposal": str(proposal_path), "approved_change_ids": [change["id"]], "mode": "magic"}), encoding="utf-8")
    ...
    def test_cli_doctor_format_validate_and_failure(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for args in (["doctor"], ["format", str(resume)], ["validate", str(resume)]):
                result = subprocess.run([sys.executable, "-m", "ats_agent.cli", *args], text=True, capture_output=True, check=False)
                self.assertEqual(result.returncode, 0, result.stderr)
                self.assertTrue(json.loads(result.stdout))
            bad = subprocess.run([sys.executable, "-m", "ats_agent.cli", "apply", str(resume)], text=True, capture_output=True, check=False)
    ...
    def test_cli_main_direct_paths(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            dataset.write_text(json.dumps({
                ...
                "expected_supported_terms": ["workflow automation"],
                "expected_unsupported_terms": [],
                "expected_hard_gates": [],
            }) + "\n", encoding="utf-8")
            self.assertEqual(cli_main(["doctor"]), 0)
            self.assertEqual(cli_main(["audit", str(resume), str(jd)]), 0)
            proposal_path = root / "proposal.json"
            self.assertEqual(cli_main(["propose", str(resume), str(jd), "--output", str(proposal_path)]), 0)
            self.assertTrue(proposal_path.exists())
            self.assertEqual(cli_main(["benchmark", "--dataset", str(dataset)]), 0)
