...
Write the user-facing result in the chat. Do not require the candidate to open JSON, HTML, or a local file. State the role, source provenance, hard gates, supported changes and variants, evidence-backed keyword coverage, and unsupported gaps in concise plain language. Keep run artifacts only as internal integrity records for the digest-bound approval flow.
...
For each unsupported but role-relevant gap, ask in chat: “Have you genuinely done work involving <gap>? If yes, what did you do, and in what setting or timeframe? Include a result or artifact if available.” A bare yes never becomes CV text. After a sufficient answer, write the truthful response to a candidate-owned supplemental evidence artifact in the run directory and rebuild a draft with the existing `--evidence` option. That artifact is candidate evidence, not a source-CV edit or approval: the rebuilt draft gets a fresh digest and still needs explicit approval before any apply step.
