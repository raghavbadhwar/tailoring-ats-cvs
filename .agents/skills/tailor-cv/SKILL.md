...
For a public job list, first run `ats-agent research-jobs "<resume>" "<jobs.json>" --candidate-id "<candidate-id>" --out "<fresh-run-directory>"`. Each item must provide an HTTPS `job_url` and may provide up to four official `context_urls`. This uses ordinary Scrapling capture only; it creates draft proposals, keyword-coverage reports, and evidence-building gap recommendations, never approvals or tailored CVs.
