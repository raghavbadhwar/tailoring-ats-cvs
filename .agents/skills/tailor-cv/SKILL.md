...
For a public job list, first run `ats-agent research-jobs "<resume>" "<jobs.json-or-career-ops-pipeline.md>" --candidate-id "<candidate-id>" --out "<fresh-run-directory>"`. JSON items must provide an HTTPS `job_url` and may provide up to four official `context_urls`; Career-Ops Markdown uses pending `- [ ] URL | Company | Role` rows and is read-only. This uses ordinary Scrapling capture only; it creates draft proposals, keyword-coverage reports, and evidence-building gap recommendations, never tracker writes, approvals, tailored CVs, or submissions.
