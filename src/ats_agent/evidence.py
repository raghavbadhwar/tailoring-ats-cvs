...
def detect_ownership(text: str) -> Ownership:
    ...
    if re.search(r"\b(founded|co-founded|owned)\b", body):
        return "owner"
    if re.search(r"\b(led|headed|directed|managed)\b", body):
        return "lead"
    if re.search(r"\b(helped|assisted|contributed|participated|collaborated|supported)\b", body):
        return "contributor"
    if re.search(r"\b(built|designed|developed|created|implemented|engineered|validated|analysed|analyzed)\b", body):
        return "direct"
    return "observed"
...
def build_evidence_ledger(candidate_id: str, sources: Iterable[EvidenceSource]) -> EvidenceLedger:
